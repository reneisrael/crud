...
class Xcrud
{
    ...
    /** !!! not available */
    public function fk_relation($fields = '', $fk_table = '', $in_fk_field = '', $out_fk_field = '', $rel_tbl = '', $rel_field =
        '', $rel_name = '', $rel_where = array(), $rel_orderby = array(), $rel_concat_separator = ' ')
    {
        if ($fields && $rel_tbl && $rel_field && $rel_name)
        {
            if ($depend_on)
    ...
    protected function _post($field = '', $default = false, $filter = false)
    {
        if (isset($_POST['xcrud'][$field]))
        {
            if (($field == 'postdata' or $field == 'unique') && $_POST['xcrud'][$field])
            {
                ...
                {
                    $data_keys[$k] = $this->fieldname_decode($key);
                }
            ...
            elseif ($filter)
            {
                switch ($filter)
                {
                    ...
                    case 'trim':
                        ...
                        break;
                    case 'base64':
                        return $this->fieldname_decode($_POST['xcrud'][$field]);
                        break;
                    default:
    ...
    protected function _build_select_list($csv = false)
    {
        ...
        foreach ($this->columns as $key => $val)
        {
            if ($val)
            {
                ...
                // in dev.
                elseif (isset($this->fk_join[$field_index]))
                {
                    if (is_array($this->join[$field_index]['rel_name']))
                    {
                        $tmp_fields = array();
    ...
    protected function create_fk_relation($name, $value = '')
    {
        if (!isset($this->fk_join[$name]))
        {
            return 'Restricted.';
        }
        $out = '';
        $tag = array(
            'tag' => 'select',
            'class' => 'xcrud-input ' . $this->theme_config('multiselect_field'),
            'data-type' => 'select',
            'name' => $name,
            'multiple' => '',
            'size' => 10);
        ...
        $db = Xcrud_db::get_instance($this->connection);
        $where_arr = array();
        if ($this->fk_join[$name]['rel_where'])
        {
            if (is_array($this->fk_join[$name]['rel_where']))
            {
                foreach ($this->fk_join[$name]['rel_where'] as $field => $val)
                {
                    $val = $this->replace_text_variables($val, $this->result_row);
                    $where_arr[] = $this->_where_field(array('field' => $field, 'table' => $this->fk_join[$name]['rel_tbl'])) . $this->
                        _cond_from_where($field) . $db->escape($val);
                }
            }
            else
            {
                $where_arr[] = $this->replace_text_variables($this->fk_join[$name]['rel_where'], $this->result_row);
            }
        }
        $out .= $this->open_tag($tag, '', array(), true);
        ...
        if (is_array($this->fk_join[$name]['rel_name']))
        {
            $name_select = 'CONCAT_WS(' . $db->escape($this->fk_join[$name]['rel_separator']) . ',`' . implode('`,`', $this->
                fk_join[$name]['rel_tbl'] . '`.`' . $this->fk_join[$name]['rel_name']) . '`) AS `name`';
        }
        else
        {
            $name_select = '`' . $this->fk_join[$name]['rel_tbl'] . '`.`' . $this->fk_join[$name]['rel_name'] . '` AS `name`';
        }
        
        if ($this->fk_join[$name]['rel_orderby'])
        {
            $order_by = $this->fk_join[$name]['rel_orderby'];
        }
        else
        {
            $order_by = '`name` ASC';
        ...
        $db->query('SELECT `' . $this->fk_join[$name]['rel_tbl'] . '`.`' . $this->fk_join[$name]['rel_field'] . '` AS `field`,' .
            $name_select . ' FROM `' . $this->fk_join[$name]['rel_tbl'] . '` ' . $where . ' GROUP BY `field` ORDER BY ' . $order_by);
        ...
        if ($fields)
        {
            ...
            else
            {
                ...
                foreach ($fields as $key => $val)
                {
                    ...
                    {
                        $tmp = explode('.', $val);
                        $field_names[$this->prefix . $tmp[0] . '.' . $this->prefix . $tmp[1]] = array('table' => $tmp[0], 'field' => $tmp[1]);
                        unset($tmp);
                    }
                }
            }
    ...
    {
        return str_replace(array('=', '/'), array('-', '_'), base64_encode($name));
    }
    public function fieldname_decode($name = '')
    {
        return str_replace('`', '', base64_decode(str_replace(array('-', '_'), array('=', '/'), $name)));
    }
}
